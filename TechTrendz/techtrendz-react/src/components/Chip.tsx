...
export const Chip = ({
    ...
    return (
        <div className={`w-fit border pr-2 rounded-full bg-inherit/90 cursor-pointer relative group h-full flex ${!variant?.success && !variant?.info && !variant?.warning && !variant?.danger && 'bg-light-100 dark:bg-dark-100/70 hover:bg-light-200 hover:dark:bg-dark-200'}
        ${variant?.success && 'bg-green-400 dark:bg-green-700'} ${variant?.info && 'bg-picton-400 dark:bg-picton-700'} ${variant?.warning && 'bg-yellow-400 dark:bg-yellow-700'} ${variant?.danger && 'bg-red-400 dark:bg-red-700'} ${extraItems && 'text-xs flex justify-center items-center h-full'}`}>
            <div className="rounded-l-full pl-2" onClick={() => { onClick && onClick() }}>{data}</div>
            {tooltip &&
            ...
            {removeable && <IconClose className={`ml-1 -mr-1.5 w-3 aspect-square flex ${iconClassName}`} onClick={() => onIconClick && onIconClick()} />}
        </div>
    )
}